...
class Email extends CI_Controller {
	...
	function index()
	{
		...
		if( isset($_POST['tid']) && $_POST['tid'] != "" ){
			$tid = $_POST['tid'];
			$_POST['gallery_url'] = base_url()."images/gallery/thumb/".$tid.".jpg";
			$detailLink = INSTALLED_PAGE_URL."?app_data={%22tid%22:%22".$tid."%22}";
		}else{
			$_POST['gallery_url'] = base_url() . "images/email/default.jpg";
		...
		// Fill in dynamic values
 		$arrFind 			= array("{BASE_URL}","{NAME}", "{GALLERYIMG}","{APPLINK}", "{DETAILLINK}", "{MESSAGE}", "{TAILGATE_TITLE}");
 		$arrReplace 		= array($baseUrl, $fromName, $_POST['gallery_url'], INSTALLED_PAGE_URL, $detailLink, $message, $title);
